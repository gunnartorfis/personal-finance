...
- **Interrupt only when unsure.** A heuristic-complete mapping with nothing to decide auto-commits to a summary. The **Import preview** is surfaced only when a column role is unmatched, or when the file would add zero new rows. A mapping that required the **AI fallback** always stops for a human nod — AI involvement *is* the uncertainty signal.
