...
**Column mapping**:
The assignment of a file's columns to the roles a Transaction needs — `date`, `amount`, `merchant`, and optional `category` — so an arbitrary bank export becomes an Upload's rows. Each role binds to exactly one column, and `amount` must be a single signed column (a file with separate debit/credit columns is unsupported in v1). A mapping is derived one of three ways, in precedence order: a **Remembered mapping**, then header heuristics, then an AI suggestion; a Member confirms or corrects it before commit.
