...
describe("DisconnectButton", () => {
  it("asks for confirmation before posting", async () => {
    ...
    render(<DisconnectButton connectionId="c1" institutionName="Landsbankinn" />)
    // First click only reveals the confirm affordance — no request yet.
    fireEvent.click(screen.getByRole("button", { name: /^disconnect landsbankinn/i }))
    expect(fetchMock).not.toHaveBeenCalled()

    fireEvent.click(screen.getByRole("button", { name: /confirm disconnect landsbankinn/i }))
    await waitFor(() =>
  ...
  it("can be cancelled without posting", () => {
    ...
    expect(screen.getByRole("button", { name: /^disconnect arion/i })).toBeInTheDocument()
    expect(fetchMock).not.toHaveBeenCalled()
  })
  ...
  it("shows an inline error when the request fails", async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: /confirm disconnect kvika/i }))
