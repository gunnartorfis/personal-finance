...
function stubFetch(overrides: { institutions?: Response; connect?: Response } = {}) {
  return vi.spyOn(globalThis, "fetch").mockImplementation((input, init) => {
    const url = String(input)
    if (url.includes("/institutions")) {
      return Promise.resolve(
        overrides.institutions ??
          new Response(JSON.stringify([{ name: "Landsbankinn", country: "IS" }, { name: "Arion", country: "IS" }])),
      )
    }
    if (url.includes("/connect") && init?.method === "POST") {
      return Promise.resolve(overrides.connect ?? new Response(JSON.stringify({ url: "https://bank/sca" })))
    }
...
describe("ConnectBank", () => {
  it("loads institutions, connects the chosen bank, and redirects to consent", async () => {
    ...
    await waitFor(() => expect(screen.getByRole("option", { name: "Arion" })).toBeInTheDocument())
    fireEvent.change(screen.getByRole("combobox"), { target: { value: "Arion" } })
    fireEvent.click(screen.getByRole("button", { name: /^connect$/i }))

    await waitFor(() => expect(assign).toHaveBeenCalledWith("https://bank/sca"))
    const connectCall = fetchMock.mock.calls.find(([u, i]) => String(u).includes("/connect") && i?.method === "POST")
  ...
  it("shows an inline error and does not navigate when connect fails", async () => {
    ...
    await waitFor(() => expect(screen.getByRole("option", { name: "Landsbankinn" })).toBeInTheDocument())
    fireEvent.click(screen.getByRole("button", { name: /^connect$/i }))
  ...
  it("surfaces a load failure instead of an empty picker", async () => {
    ...
    render(<ConnectBank />)
    expect(await screen.findByRole("alert")).toHaveTextContent(/couldn.t load banks/i)
  })
})
