...
| 2a | **Tool layer — cycle analytics**: tool contract (`AssistantTool`/context) + `getCycleSummary`, `spendByType`, `topMerchants`, `compareCycles` (the "why was March higher" core). zod in/out, tenant-scoped, no LLM. | new `lib/assistant/tools/*` | PR-OPEN #260 |
| 2b | **Tool layer — search + status**: `searchTransactions`, `getSpendingTrend`, `getSavingsStatus`, `getFinancialHealth`; append to the `assistantTools` registry. zod in/out, tenant-scoped, no LLM. | `lib/assistant/tools/*` | PR-OPEN #262 |
