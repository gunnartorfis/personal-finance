...
| 2a | **Tool layer — cycle analytics**: tool contract (`AssistantTool`/context) + `getCycleSummary`, `spendByType`, `topMerchants`, `compareCycles` (the "why was March higher" core). zod in/out, tenant-scoped, no LLM. | new `lib/assistant/tools/*` | PR-OPEN |
