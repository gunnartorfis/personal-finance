...
| 3a | **API core (`lib/assistant`)**: config consts (`ASSISTANT_MODEL`/max-steps/daily-cap), system-prompt builder (Locale + today + current cycle + domain vocab + guardrails), AI-SDK tool adapter (`toAiTools`), daily-cap helper. Unit-tested; no route. | `lib/assistant/*` | PR-OPEN #263 |
